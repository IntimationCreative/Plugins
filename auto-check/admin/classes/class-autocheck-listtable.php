 ...
 * @since 1.0
 */
class Autocheck_List_Table extends WP_List_Table
{
    public $items;
	...
	protected function column_name( $item ) {
		...
		if ( empty( $title ) ) {
			$title = '<em>' . esc_html__( 'Untitled', 'autocheck' ) . '</em>';
		}
		...
		$actions = array(
			'edit' => sprintf( '<a href="%s">%s</a>', esc_url( $edit_link ), esc_html__( 'Edit', 'autocheck' ) ),
			'delete' => sprintf( '<a href="%s">%s</a>', esc_url( $delete_link ), esc_html__( 'Delete', 'autocheck' ) ),
		);
	...
	function column_update( $item ) {
		...
		return sprintf( '<a href="%s" class="%s">%s</a>', esc_url( $update_link ), 'custombtn no-margin', esc_html__( 'Update', 'autocheck' ) );
	}
}
